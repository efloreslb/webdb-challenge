...
1. Explain the difference between `RDBMS` and `SQL`.
    - SQL is the language used to handle databases while a RDBMS is the software(system) used to handle the database.
1. Why do tables need a `primary key`?
    - Table records need to be identified by a unique identifier that does not repeat to be able to pull that record when needed.
1. What is the name given to a table column that references the primary key on another table.
    - Foreign Key
1. What do we need in order to have a _many to many_ relationship between two tables.
    - Both tables must reference multiple records in the other table.
